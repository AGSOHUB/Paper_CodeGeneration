...
def get_all_elements_from_codebase(global_only=True):
    """
    Extracts all defined functions, variables, typedefs, and structs from a C source file 
    and its associated files by preprocessing the code and analyzing its Abstract Syntax Tree (AST).

    Parameters:
    - global_only (bool): If `True`, only extracts globally defined elements. If `False`, 
      includes local elements as well.

    Returns:
    - dict: A dictionary containing lists of all extracted elements:
