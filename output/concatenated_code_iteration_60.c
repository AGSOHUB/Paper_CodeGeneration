 ...
 *
 * This function sets or clears a specific GPIO pin by directly manipulating
 * the Output Data Register (ODR) of the specified GPIO port. It ensures atomic
 * operations by using bitwise operations.
 *
 ...
 *
 * This function reads the input state of a specific GPIO pin by directly accessing
 * the Input Data Register (IDR) of the specified GPIO port. It calculates the pin
 * position from the provided pin mask and uses bitwise operations to determine the
 * pin state.
 *
 * @param gpio_port_base The base address of the GPIO port (e.g., 0x40020000 for GPIOA).
 * @param pin_mask A bitmask representing the pin to read (e.g., 1 << 5 for pin 5).
 * @return The state of the pin (0 if low, 1 if high).
 */
