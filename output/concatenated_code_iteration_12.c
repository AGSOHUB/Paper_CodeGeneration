...
void ENABLE_GPIOA_CLOCK(void) {
    ...
    *RCC_AHB1ENR |= (1 << 0);
    // Optionally, you can add a delay or a check to ensure the clock is enabled
    // This is a simple check to ensure the clock is enabled
    while ((*RCC_AHB1ENR & (1 << 0)) == 0) {
 ...
 * 
 * This function writes a high or low value to a GPIO pin on the STM32F407 board
 * by directly manipulating the Output Data Register (ODR) of the GPIO port.
 * 
 * @param gpio_port_base The base address of the GPIO port (e.g., 0x40020000 for GPIOA).
 ...
 * It uses direct register access to configure the pin as either input or output.
 * 
 * @param gpio_base The base address of the GPIO port (e.g., 0x40020000 for GPIOA).
 * @param pin_mask The bitmask representing the pin to configure (e.g., 1 << 5 for pin 5).
...
void set_input_output_mode(uint32_t gpio_base, uint32_t pin_mask, uint8_t mode) {
    ...
    *GPIO_MODER |= (mode << (pin_number * 2));
    ...
    // Set or clear the pin based on the mode
    if (mode) {
        // Set the pin high if mode is output
        *GPIO_ODR |= (1 << pin_number);
    } else {
        // Set the pin low if mode is input
        *GPIO_ODR &= ~(1 << pin_number);
    }
}
...
int hardware_abstraction_layer_function_gpio_read_pin(uint32_t gpio_port_base, uint32_t pin_mask) {
    ...
    return ((*GPIO_IDR & pin_mask) != 0) ? 1 : 0;
}
