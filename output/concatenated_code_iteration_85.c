 ...
 * 
 * This function sets or clears a specified GPIO pin by directly manipulating the
...
int hardware_abstraction_layer_function_gpio_read_pin(uint32_t port_base, uint16_t pin) {
    ...
    volatile uint32_t *IDR = (uint32_t *)(port_base + 0x10);
...
void set_input_output_mode(uint32_t port_base, uint16_t pin, uint32_t mode) {
    ...
    // Pointer to the Mode Register (MODER) of the specified port
    volatile uint32_t *moder = (volatile uint32_t *)(port_base + 0x00);

    // Clear the existing mode for the pin (2 bits per pin)
