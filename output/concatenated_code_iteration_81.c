 ...
 *
 * This function directly manipulates the Output Data Register (ODR) of a specified GPIO port
 * to set or clear the state of a specific pin. It uses bitwise operations to ensure atomic
 * and reliable operations, which is crucial in embedded systems to avoid race conditions.
 *
 * @param gpio_port_base The base address of the GPIO port (e.g., 0x40020000 for GPIOA).
...
void hardware_abstraction_layer_function_gpio_write_pin(uint32_t gpio_port_base, uint32_t pin_mask, uint32_t value) {
    ...
    if (value) {
        ...
        *ODR |= (1 << pin_position);
        ...
        // Clear the pin: use bitwise AND with NOT to clear the bit at pin_position
        *ODR &= ~(1 << pin_position);
    }
}

/**
 * @brief Sets the mode of a GPIO pin to either input or output on the STM32F407 board.
 *
 * This function configures a specific GPIO pin as either an input or an output.
 * It directly manipulates the GPIO mode register (MODER) to set the desired mode.
 *
