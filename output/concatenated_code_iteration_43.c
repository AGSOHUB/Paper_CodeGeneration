...
int hardware_abstraction_layer_function_gpio_read_pin(uint32_t port_base, uint16_t pin) {
    ...
    // Read the state of the pin using bitwise operations
 ...
 * 
 * This function writes a value (0 or 1) to a specific GPIO pin by directly manipulating the
 * Output Data Register (ODR) of the specified GPIO port. It ensures atomic
 * and reliable operations using bitwise operations.
 * 
 * @param port_base The base address of the GPIO port (e.g., GPIOA, GPIOB).
 * @param pin The specific pin number to write to (e.g., GPIO_PIN_1, GPIO_PIN_2).
...
void ENABLE_GPIOC_CLOCK(void) {
    ...
    // Get the address of the AHB1ENR register
...
void ENABLE_GPIOB_CLOCK(void) {
    ...
    // Get the address of the AHB1ENR register
    volatile uint32_t *RCC_AHB1ENR = (volatile uint32_t *)(RCC_BASE + RCC_AHB1ENR_OFFSET);
    // Enable the clock for GPIOB by setting the corresponding bit
 ...
 * 
 * This function creates a delay for a specified number of milliseconds by
 * performing a busy-wait loop. It assumes a system clock of 16 MHz and that
 * the SysTick timer is configured to generate an interrupt every 1 ms.
 * 
